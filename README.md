...
### Pivot Selectivity
The sort carefully selects the pivot from the middle of five sorted candidates. These candidates allow us to determine whether the data in the current interval is approximately descending and inform our "partition left" strategy.

### Introspection
The sort is introspective, switching to a guaranteed nlog(n) sort if the sort becomes quadratic. Like PDQsort, the sort switches to Heapsort after log(n) "bad" partitions&mdash; partitions that are significantly unbalanced.
